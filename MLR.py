...
<div class="card">
            ...
            <p><b>co-efiicient(Total bill):</b> {model.coef_[0]:.3f}<br>
            <p><b>co-efiicient(Group size):</b> {model.coef_[1]:.3f}<br>
            <b><b>Intercept</b>{model.intercept_:.3f}</p>
            </div>
""",unsafe_allow_html=True)
#Predictions
st.markdown('<div class="card">', unsafe_allow_html=True)
st.subheader("Predict Tip Amount")
bill = st.slider("Total Bill Amount",min_value=float(df["total_bill"].min()), max_value=float(df["total_bill"].max()),value=30.0)
